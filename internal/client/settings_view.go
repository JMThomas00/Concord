...
func (a *App) renderServerManagementView() string {
	...
	catHeaderStyle := lipgloss.NewStyle().
		...
		Bold(true).
		Width(catWidth - 2)
	catBuf.WriteString(catHeaderStyle.Render("SERVER SETTINGS"))
	catBuf.WriteString("\n")
	catBuf.WriteString(lipgloss.NewStyle().
		Foreground(lipgloss.Color(a.theme.Colors.Comment)).
		Width(catWidth - 2).
	...
	headerStyle := lipgloss.NewStyle().
		Foreground(lipgloss.Color(a.theme.Colors.Cyan)).
		Bold(true)
	contentBuf.WriteString(headerStyle.Render(s.Categories[s.SelectedCategory]))
	contentBuf.WriteString("\n\n")
	contentBuf.WriteString("Server management features - Coming soon\n")
